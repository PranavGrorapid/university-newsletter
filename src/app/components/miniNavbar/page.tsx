...
const MiniNavbar = ({ navLinks }: any) => {
  ...
  // using the useEffect hook to calculate the number of links that should be displayed according to device configuration
  ...
  const displayedOptions = navLinks
    ? navLinks.slice(currentCard, currentCard + cardsToShow)
    : [];
  ...
  return (
    <div className="bg-primaryBackground py-5 px-4 flex items-center justify-start space-x-2 sm:space-x-4 lm:space-x-8 md:space-x-10 md:justify-center md:px-6 nxl:space-x-12 xl:space-x-15 xl:px-8">
      ...
      {/* when we click on this button next set of links will come */}
